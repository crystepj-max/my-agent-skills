...
description: "仅在用户明确指定 dev-workflow-2-0 或开发工作流 2.0、或继续已有旧版运行时使用；依赖 DSH workflow 工具。新建完整功能交付默认使用 construction-bootstrap，不因普通 issue 或开发请求自动选择旧流程。"
---

# 开发工作流 2.0 · DSH 运行技能

仅在明确指定旧版流程的 DSH 会话中运行「开发工作流 2.0」。本 skill 目录自包含：
